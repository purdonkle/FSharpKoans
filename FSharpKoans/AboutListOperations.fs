    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec findLen xs len = // write a function to find the length of a list
                match xs with
                | [] -> len
                | _::rest -> findLen rest (len + 1)
            findLen xs 0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec reverseList xs reversed = // write a function to reverse a list here.
                match xs with
                | [] -> reversed
                | a::rev -> reverseList rev (a::reversed)
            reverseList xs []
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec doubleElements xs out =
                match xs with
                | [] -> List.rev out
                | a::rest -> doubleElements rest (a*2::out)
            doubleElements xs []
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec applyFunction xs out =
                match xs with
                | [] -> List.rev out
                | a::rest -> applyFunction rest ((f a)::out)
            applyFunction xs []
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filterFunc xs out =
                match xs with
                | [] -> List.rev out
                | a::rest ->
                    match (f a) with
                    | true -> filterFunc rest (a::out)
                    | false -> filterFunc rest out
            filterFunc xs []
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec filterOdd xs out =
                match xs with
                | [] -> List.rev out
                | a::rest -> 
                    match (a%2 = 0) with
                    | false -> filterOdd rest (a::out)
                    | true -> filterOdd rest out
            filterOdd xs []
        ...
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filterFunc xs out =
                ...
                | a::rest ->
                    ...
                    | true -> filterFunc rest (a::out)
                    | false -> filterFunc rest out
            filterFunc xs [] // write a function which filters based on the specified criteria
        filter (fun x -> x > 19) [9; 5; 23; 66; 4] |> should equal [23; 66]
        filter (fun x -> String.length x = 4) ["moo"; "woof"; "yip"; "nyan"; "meow"]
        |> should equal ["woof"; "nyan"; "meow"]
        filter (fun (a,b) -> a*b >= 14) [9,3; 4,2; 4,5] |> should equal [9,3; 4,5]
...
*)
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec multiplyFolder state xs =
                match xs with
                | [] -> state
                | a::rest -> multiplyFolder (state * a) rest
            multiplyFolder initialState xs
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec functionFolder state xs =
                match xs with
                | [] -> state
                | a::rest -> functionFolder (f state a) rest
            functionFolder initialState xs
    ...
    let ``18 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec existFolder xs =
                match xs with
                | [] -> false
                | a::rest ->
                    match (f a) with
                    | true -> true
                    | false -> existFolder rest 
            existFolder xs
    ...
    let ``19 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec partitionFolder xs a b =
                match xs with
                | [] -> (List.rev a, List.rev b)
                | head::rest ->
                    match (f head) with
                    | false -> partitionFolder rest a (head::b)
                    | true -> partitionFolder rest (head::a) b
            partitionFolder xs [] []
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec initFolder counter list =
                match (counter < n) with
                | false -> List.rev list
                | true -> initFolder (counter + 1) (f counter::list)
            initFolder 0 []
    ...
    let ``21 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec tryFindFolder xs =
                match xs with
                | [] -> None
                | a::rest ->
                    match (p a) with
                    | true -> Some a
                    | false -> tryFindFolder rest
            tryFindFolder xs
    ...
    let ``22 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec tryPickFolder xs =
                match xs with
                | [] -> None
                | a::rest ->
                    match (p a) with
                    | None -> tryPickFolder rest
                    | _ -> p a
            tryPickFolder xs
    ...
    *)
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec chooseFolder xs out =
                match xs with
                | [] -> List.rev out
                | a::rest -> 
                    match (p a) with
                    | None -> chooseFolder rest out
                    | Some a -> chooseFolder rest (a::out)
            chooseFolder xs []
    ...
    let ``24 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec mapiFolder counter xs out =
                match xs with
                | [] -> List.rev out
                | a::rest -> mapiFolder (counter + 1) rest ((f counter a)::out)
            mapiFolder 0 xs []
